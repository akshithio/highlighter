...
(() => {
  ...
  let paletteOverride: "light" | "dark" | null = null;
  let dragState = null;
  let sideDragState = null;
  let currentPaletteTheme: "light" | "dark" = "light";
  ...
  function updateHighlightNoteState(id, hasNote) {
    document.querySelectorAll(`.${HIGHLIGHT_CLASS}[data-page-notes-id="${CSS.escape(id)}"]`).forEach((node) => {
      (node as HTMLElement).dataset.hasNote = String(hasNote);
    });
  }

  function updateHighlightColor(id, color) {
    document.querySelectorAll(`.${HIGHLIGHT_CLASS}[data-page-notes-id="${CSS.escape(id)}"]`).forEach((node) => {
      (node as HTMLElement).style.setProperty("--page-notes-highlight-color", color);
      (node as HTMLElement).style.setProperty("background-color", color, "important");
    });
  }
  ...
  function showSideComments() {
    ...
    const notes = pageData.highlights
      .map((highlight) => ({ highlight, marker: getFirstHighlightNode(highlight.id) }))
      .filter((item) => item.highlight.note && item.marker)
      .map((item) => ({ ...item, rect: item.marker.getBoundingClientRect() }))
    ...
    for (const { highlight, rect } of notes) {
      ...
      card.addEventListener("pointermove", (event) => {
        ...
        sideDragState.moved =
          sideDragState.moved ||
          Math.abs(event.clientY - sideDragState.startY) > 3 ||
          Math.abs(event.clientX - sideDragState.startX) > 3;
      });
  ...
  colorOptions.addEventListener("click", async (event) => {
    const option = (event.target as Element | null)?.closest?.(".page-notes-color-option") as HTMLElement | null;
    if (!option) return;
  ...
  document.addEventListener("pointerdown", (event) => {
    ...
    const marker = target?.closest?.(`.${HIGHLIGHT_CLASS}`) as HTMLElement | null;
    ...
    const sideCard = target?.closest?.(".page-notes-side-card") as HTMLElement | null;
    if (sideCard) {
      event.preventDefault();
      hideToolbar();
      return;
